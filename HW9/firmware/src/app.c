    ...
    switch (appData.state) {
        ...
        case APP_STATE_SCHEDULE_WRITE:
            ...
            if (appData.isReadComplete) {
                ...
                if (index == 1) {
                    if (i < 100) {
                        I2C_read_multiple(SLAVE, OUT_TEMP_L, *data, 14); // get the data and combine into the 7 shorts
                        temp = combine_data(data,0);
                        gyro_x = combine_data(data,2);
                        gyro_y = combine_data(data,4);
                        gyro_z = combine_data(data,6);
                        ...
                        accel_y = combine_data(data,10);
                        accel_z = combine_data(data,12);
                        // write data in the form "index ax ay az gx gy gz"
                        len = sprintf(dataOut,"%d %d %d %d %d %d %d", i, accel_x, accel_y, accel_z, gyro_x, gyro_y, gyro_z); 
                        USB_DEVICE_CDC_Write(USB_DEVICE_CDC_INDEX_0,
                            &appData.writeTransferHandle, dataOut, len,
                            USB_DEVICE_CDC_TRANSFER_FLAGS_DATA_COMPLETE);
                        i++;
                    }
                    else { // set index to 0 once 100 values are sent
                        i = 0;
                        index = 0;
                        USB_DEVICE_CDC_Write(USB_DEVICE_CDC_INDEX_0,
